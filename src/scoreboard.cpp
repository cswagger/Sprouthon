...
void update_scoreboard_score() {
    ...
    if (apple.x == python.x[0] && apple.y == python.y[0]) {
        ...
        if (apple.color == 0) {
            score_board.score_num[0] += 3 ;
            score_board.score_num[1] += 1 ;
        } else if (apple.color == 1) {
            score_board.score_num[0] += 2 ;
            score_board.score_num[2] += 1 ;
        } else if (apple.color == 2) {
            score_board.score_num[0] += 1 ;
            score_board.score_num[3] += 1 ;
        }
        spawn_apple() ;
    }
}